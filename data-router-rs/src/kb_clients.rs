...
use crate::router::{AgentScopeManager, create_scope_violation_error, add_scope_metadata_to_response};
...
impl MindKbClient {
    ...
    pub async fn query_kb(&self, mut query_req: QueryRequest, query_meta: QueryMetadata) 
        -> Result<Response<QueryResponse>, Status> {
        
        // First validate if the agent can make this query
        let validation_result = self.scope_manager.validate_query(&query_meta.agent_id, &query_req).await;
        
        match validation_result {
            crate::router::ScopeVerificationResult::Allowed => {
                // Apply scope filter to limit results to accessible scopes
                query_req = self.scope_manager.apply_scope_filter(&query_meta.agent_id, query_req).await;
                
                // Get the accessible scopes for debugging/audit
                let accessible_scopes = self.scope_manager.get_accessible_scopes(&query_meta.agent_id).await;
    ...
    pub async fn store_fact(&self, mut store_req: StoreRequest, query_meta: QueryMetadata) 
        ...
        if !metadata.contains_key("scope") {
            ...
            if !accessible_scopes.is_empty() {
                ...
                let default_scope = accessible_scopes.iter()
                    .find(|s| s != &"PUBLIC")
        ...
        if !self.scope_manager.can_access_scope(&query_meta.agent_id, &target_scope).await {
            return Err(create_scope_violation_error(&query_meta.agent_id, &target_scope));
        }
    ...
    pub async fn retrieve(&self, retrieve_req: RetrieveRequest, query_meta: QueryMetadata) 
        ...
        if let Some(fact) = &result.fact {
            if let Some(metadata) = &fact.metadata {
                if let Some(scope) = metadata.get("scope") {
                    if !self.scope_manager.can_access_scope(&query_meta.agent_id, scope).await {
                        // Don't reveal that the item exists, return not found
                        return Err(Status::not_found(format!("Fact with ID {} not found", retrieve_req.id)));
                    }
                }
            }
        }
...
mod tests {
    ...
    async fn test_scope_filtering() {
        ...
        let filtered_query = scope_manager.apply_scope_filter("RED-TEAM-SHADOW", query).await;
