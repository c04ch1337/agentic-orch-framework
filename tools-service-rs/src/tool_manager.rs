...
use crate::validation::{ToolValidationError, validate_command_name};
    ...
    fn from(error: tool_sdk::error::ServiceError) -> Self {
        match error {
            tool_sdk::error::ServiceError::Authentication(msg) =>
                ToolManagerError::ValidationError(ToolValidationError::Other(format!("Authentication error: {}", msg))),
            tool_sdk::error::ServiceError::Authorization(msg) =>
                ToolManagerError::ValidationError(ToolValidationError::Other(format!("Authorization error: {}", msg))),
...
pub static TOOL_MANAGER: Lazy<Arc<ToolManager>> = Lazy::new(|| {
...
impl ToolManager {
    ...
    pub fn register_tool(&self, tool: Arc<dyn Tool>) -> Result<(), ToolManagerError> {
        ...
        info!("Tool {} v{} registered successfully", metadata.name, metadata.version);
        Ok(())
    }
    ...
    pub fn check_tool_capabilities(&self, tool_id: &str, required: &HashSet<Capability>) -> Result<(), ToolManagerError> {
    ...
    pub async fn grant_session_capabilities(&self, session_id: &str, capabilities: HashSet<Capability>) {
        let mut session_grants = self.session_grants.lock().await;
        
        let session_capabilities = session_grants.entry(session_id.to_string()).or_insert_with(HashSet::new);
        for cap in capabilities {
            session_capabilities.insert(cap);
        }
        
        debug!("Granted capabilities to session {}: {:?}", session_id, session_capabilities);
    }
    
    /// Check if a session has the required capabilities
    pub async fn check_session_capabilities(&self, session_id: &str, required: &HashSet<Capability>) -> Result<(), ToolManagerError> {
        ...
        match session_grants.get(session_id) {
            ...
            None => Err(ToolManagerError::MissingCapability("Session has no capabilities".to_string())),
        }
    }
    
    /// Execute a tool with the given context
    pub async fn execute_tool(&self, tool_id: &str, context: ToolContext) -> Result<ToolResult, ToolManagerError> {
        // Get the tool
        ...
        if let Some(session_id) = &context.session_id {
            ...
            self.check_session_capabilities(session_id, &required_capabilities).await?;
        }
    ...
    pub fn load_tools_from_directory(&self, directory: &str) -> Result<Vec<String>, ToolManagerError> {
        ...
        Err(ToolManagerError::LoadingError("Dynamic loading not implemented yet".to_string()))
    }
    ...
    pub fn validate_tool_metadata(&self, metadata: &ToolMetadata) -> Result<(), ToolManagerError> {
        ...
        if metadata.id.is_empty() {
            return Err(ToolManagerError::InvalidMetadata("Tool ID cannot be empty".to_string()));
        }
        
        if metadata.name.is_empty() {
            return Err(ToolManagerError::InvalidMetadata("Tool name cannot be empty".to_string()));
        }
        
        if metadata.version.is_empty() {
            return Err(ToolManagerError::InvalidMetadata("Tool version cannot be empty".to_string()));
        }
        
        // Validate version format (should be semver)
        if !metadata.version.contains('.') {
            return Err(ToolManagerError::InvalidMetadata("Tool version should follow semver format".to_string()));
        }
        
        // Check parameters
        for param in &metadata.parameters {
            if param.name.is_empty() {
                return Err(ToolManagerError::InvalidMetadata("Parameter name cannot be empty".to_string()));
            }
            ...
            if let Some(validation) = &param.validation {
                if let Err(_) = regex::Regex::new(validation) {
                    return Err(ToolManagerError::InvalidMetadata(
                        format!("Invalid validation pattern for parameter {}: {}", param.name, validation)
                    ));
                }
            }
        }
            ...
            for param in &self.metadata.parameters {
                if param.required && !parameters.contains_key(&param.name) {
                    return Err(ToolManagerError::ValidationError(
                        ToolValidationError::Other(format!("Missing required parameter: {}", param.name))
                    ));
                }
            }
