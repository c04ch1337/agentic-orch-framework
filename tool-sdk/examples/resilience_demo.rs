...
use tool_sdk::{
    resilience::{Resilience, RetryConfig, CircuitBreakerConfig},
    error::{ServiceError, Result},
};
...
async fn main() -> Result<()> {
    ...
    let result = resilience.execute(move || {
        ...
        async move {
            ...
            if attempt <= 2 {
                ...
                Err(ServiceError::network(format!("Temporary failure in attempt {}", attempt)))
            } else {
    ...
    println!("Circuit status after wait: {:?}", resilience.circuit_breaker_status());
