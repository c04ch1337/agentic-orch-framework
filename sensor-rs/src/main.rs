...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let body_kb_addr = env::var("BODY_KB_ADDR").unwrap_or_else(|_| DEFAULT_BODY_KB_ADDR.to_string());

    // Ensure address has http:// prefix for tonic
    let body_kb_addr = if body_kb_addr.starts_with("http://") || body_kb_addr.starts_with("https://") {
        body_kb_addr
    } else {
        format!("http://{}", body_kb_addr)
    };

    println!("sensor-rs starting - streaming to Body-KB at {}", body_kb_addr);
