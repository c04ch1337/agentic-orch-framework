...
use crate::agi_core::{UserIdentity, RegisterUserRequest};
...
pub fn validate_role(role: i32) -> ValidationResult<i32> {
    ...
    if role < 0 || role > 3 {
        return Err(format!("Invalid role: {}. Must be between 0 and 3", role));
    }
...
pub fn validate_permissions(permissions: &[String]) -> ValidationResult<Vec<String>> {
    if permissions.len() > MAX_PERMISSIONS_COUNT {
        return Err(format!("Too many permissions: {}, max allowed: {}", 
            permissions.len(), MAX_PERMISSIONS_COUNT));
    }
...
pub fn validate_attributes(attributes: &HashMap<String, String>) -> ValidationResult<HashMap<String, String>> {
    if attributes.len() > MAX_ATTRIBUTES_COUNT {
        return Err(format!("Too many attributes: {}, max allowed: {}", 
...
pub fn validate_register_user_request(request: &RegisterUserRequest) -> ValidationResult<RegisterUserRequest> {
    // Check if identity is present
    let identity = match &request.identity {
        Some(identity) => identity,
        None => return Err("Missing user identity".to_string()),
    };
...
pub fn validate_role_filter(role_filter: i32) -> ValidationResult<i32> {
    ...
    if role_filter < -1 || role_filter > 3 {
        return Err(format!("Invalid role filter: {}. Must be between -1 and 3", role_filter));
    }
