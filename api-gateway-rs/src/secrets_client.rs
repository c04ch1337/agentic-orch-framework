...
use secrets_service::{SecretRequest, TokenRequest, ServiceAuthRequest};
...
impl SecretsClient {
    ...
    pub async fn new() -> Result<Self, SecretsError> {
        ...
        let client = match client_result {
            ...
            Err(err) => {
                ...
                log::warn!("Starting in degraded mode - secrets operations will use default API key");
                None
            }
        };
    ...
    async fn authenticate(&self) -> Result<String, SecretsError> {
        ...
        let client_ref = self.client.as_ref()
            .ok_or_else(|| SecretsError::ConfigurationError(
                "No secrets service client available".to_string()
            ))?;
        ...
        let request = Request::new(TokenRequest {
            ...
            ttl: 1800, // 30 minutes
            roles: vec!["api-gateway".to_string(), "read".to_string(), "write".to_string()],
        });
        ...
        if self.is_mock() {
            if key == "api-key/default" {
                let api_key = env::var("API_KEY")
                    .unwrap_or_else(|_| "phoenix-default-key".to_string());
            ...
            Err(status) => {
                ...
                if status.code() == tonic::Code::NotFound {
                    ...
                    *token_writer = None;
                    ...
                    Err(SecretsError::AuthenticationError(status.message().to_string()))
                } else {
                    Err(SecretsError::GrpcError(status))
                }
            }
    ...
    pub async fn verify_token(&self, token: &str) -> Result<TokenData, SecretsError> {
        ...
        if self.is_mock() {
            ...
            let default_key = env::var("API_KEY")
                .unwrap_or_else(|_| "phoenix-default-key".to_string());
            ...
            if token == default_key {
                ...
                return Err(SecretsError::AuthenticationError("Invalid token".to_string()));
            }
        }
    ...
    pub async fn generate_client_token(&self) -> Result<TokenData, SecretsError> {
        ...
        // Cache the token
        let mut cache = self.token_cache.write().await;
        cache.insert(token.clone(), CachedToken {
            token_data: token_data.clone(),
            created_at: Instant::now(),
        });
