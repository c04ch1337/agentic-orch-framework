...
// Import dependencies
use auth_middleware::{
    auth_middleware, generate_client_token, init_auth_client, is_auth_healthy,
    permission_middleware,
};
use phoenix_auth::{load_api_keys_from_file, phoenix_auth_interceptor};
use rate_limit::tower_governor_rate_limiter;
use secrets_client::{SecretsClient, SecretsError};
use validation::{
    payload_limit_config, sanitize_json_input, sanitize_request, validate_content_type,
    validate_request, ApiValidationError, ValidationErrorResponse, MAX_PAYLOAD_SIZE,
};
...
use agi_core::{orchestrator_service_client::OrchestratorServiceClient, Request as ProtoRequest};
...
fn is_plan_and_execute(request: &ExecuteRequest) -> bool {
    ...
    let method_indicates_plan = matches!(method.as_str(), "plan_and_execute" | "orchestrated_chat");
...
async fn validate_request_middleware(
    ...
    let body_bytes = match to_bytes(body, MAX_PAYLOAD_SIZE).await {
        ...
        Err(e) => {
            let error =
                ApiValidationError::InvalidFormat(format!("Failed to read request body: {}", e));
    ...
    if body_bytes.len() > MAX_PAYLOAD_SIZE {
        let error = ApiValidationError::PayloadTooLarge(format!(
    ...
    let sanitized_body = match serde_json::to_string(&json_value) {
        Ok(body) => body,
        Err(err) => {
            let error = ApiValidationError::InvalidFormat(format!(
                "Failed to serialize sanitized request: {}",
                err
            ));
            return Err(error.to_response());
        }
    ...
    let req_builder = axum::http::Request::builder().uri(uri).method(method);
...
async fn execute_handler(
    ...
    match client_result {
        Ok(mut client) => {
            ...
            if is_plan {
                match client
                    .plan_and_execute(tonic::Request::new(proto_request))
                    .await
                {
                    Ok(response) => {
                ...
                match client
                    .process_request(tonic::Request::new(proto_request))
                    .await
                {
                    Ok(response) => {
                        let inner = response.into_inner();
...
async fn generate_token_handler(headers: HeaderMap) -> impl IntoResponse {
    ...
    match generate_client_token().await {
        Ok(token_data) => Json(serde_json::json!({
...
async fn load_tls_config() -> Result<Option<RustlsConfig>, Box<dyn std::error::Error>> {
    ...
    let cert_path =
        env::var("TLS_CERT_PATH").unwrap_or_else(|_| "certs/api-gateway.pem".to_string());
    let key_path = env::var("TLS_KEY_PATH").unwrap_or_else(|_| "certs/api-gateway.key".to_string());

    log::info!(
        "Loading TLS configuration from cert: {} and key: {}",
        cert_path,
        key_path
    );
...
fn create_self_signed_certificates(
    cert_path: &str,
    key_path: &str,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Err(err) = load_api_keys_from_file(&api_keys_file).await {
        log::warn!(
            "Failed to load API keys from file: {}. Using default keys.",
            err
        );
    }
    ...
    let secrets_client = match SecretsClient::new().await {
        Ok(client) => {
            ...
            if client.is_mock() {
                log::warn!(
                    "Secrets service running in mock mode - falling back to environment variables"
                );
            }
            Some(client)
        }
        Err(err) => {
            log::warn!(
                "Failed to connect to secrets service: {}. Using environment variable for API key.",
                err
            );
            None
        }
    ...
    if let Some(client) = &secrets_client {
        match client.get_default_api_key().await {
            ...
            Err(err) => log::warn!(
                "Failed to retrieve default API key: {}. Using environment variable.",
                err
            ),
        }
    }
