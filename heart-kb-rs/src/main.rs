...
mod validation;
use validation::{
    validate_query, validate_retrieve_request, validate_sentiment_fact, validate_source_id,
...
impl HeartKbService for HeartKBServer {
    async fn query_kb(
        ...
        log::info!(
            "Heart-KB query returned {} emotional/motivational result(s)",
            results.len()
        );
    ...
    async fn retrieve(
        ...
        let value = format!(
            "Heart-KB retrieved emotional state for key: '{}' - Happiness: 0.7, Curiosity: 0.9",
            req_data.key
        )
        .into_bytes();
        ...
        log::info!(
            "Heart-KB retrieved emotional state for key: {}",
            req_data.key
        );
    ...
    async fn store_sentiment(
        ...
        let fact = req
            .fact
            .ok_or_else(|| Status::invalid_argument("Missing sentiment fact"))?;
        ...
        log::info!(
            "Heart-KB StoreSentiment: source={}, sentiment={:?}, confidence={}",
            source_id,
            new_sentiment,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let addr_str = env::var("HEART_KB_ADDR").unwrap_or_else(|_| "0.0.0.0:50059".to_string());
