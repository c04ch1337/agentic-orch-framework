...
use winapi::um::securitybaseapi::{
    AllocateAndInitializeSid, FreeSid, GetLengthSid, SetTokenInformation,
};
use winapi::um::synchapi::WaitForSingleObject;
use winapi::um::winbase::{
    CREATE_NEW_PROCESS_GROUP, CREATE_SUSPENDED, CREATE_UNICODE_ENVIRONMENT, STARTF_USESHOWWINDOW,
...
impl JobObjectManager {
    ...
    pub async fn execute_code(
        ...
        let mut startup_info =
            unsafe { std::mem::zeroed::<winapi::um::processthreadsapi::STARTUPINFOW>() };
        ...
        let mut process_info =
            unsafe { std::mem::zeroed::<winapi::um::processthreadsapi::PROCESS_INFORMATION>() };
        ...
        let assign_result =
            unsafe { AssignProcessToJobObject(self.job_handle, process_info.hProcess) };
        ...
        log::info!(
            "Process assigned to Job Object with PID: {}",
        ...
        let wait_result =
            unsafe { WaitForSingleObject(process_handle, EXECUTION_TIMEOUT_MS as DWORD) };
        ...
        let (stdout, stderr, exit_code) = match wait_result {
            ...
            WAIT_TIMEOUT => {
                ...
                log::warn!(
                    "Process timeout after {} seconds, terminating",
                    EXECUTION_TIMEOUT_MS / 1000
                );
                unsafe {
    ...
    fn set_low_integrity_level(&self, process_handle: HANDLE) -> Result<(), String> {
        ...
        let set_result = unsafe {
            SetTokenInformation(
                ...
                std::mem::size_of::<TOKEN_MANDATORY_LABEL>() as DWORD + GetLengthSid(low_sid),
            )
        };
        ...
        if set_result == 0 {
            ...
            return Err(format!(
                "Failed to set Low Integrity Level: error code {}",
                error
            ));
        }
        ...
        thread::spawn(move || {
            ...
            loop {
                ...
                if start_time.elapsed() > Duration::from_millis(EXECUTION_TIMEOUT_MS) {
                    log::error!(
                        "CRITICAL_RESOURCE_BREACH: Process {} exceeded 10s timeout",
                        process_id
                    );
                    unsafe {
                ...
                if let Ok(job_info) = job_handle.query_extended_limit_info() {
                    ...
                    if job_info.peak_process_usage > MAX_CPU_RATE as u64 {
                        log::error!(
                            "CRITICAL_RESOURCE_BREACH: Process {} exceeded CPU limit of 50%",
                            process_id
                        );
                        unsafe {
                            TerminateJobObject(dup_job_handle, 888);
                    ...
                    if job_info.peak_job_memory_used > MAX_JOB_MEMORY as u64 {
                        log::error!(
                            "CRITICAL_RESOURCE_BREACH: Process {} exceeded memory limit of 512MB",
                            process_id
                        );
                        unsafe {
                            TerminateJobObject(dup_job_handle, 888);
                    ...
                    if start_time.elapsed().as_secs() % 1 == 0 {
                        log::info!(
                            "Process {} resource usage - CPU: {}%, Memory: {}MB",
                            process_id,
                ...
                if let Ok(job_handle) = JobObjectHandle::new(self.job_handle) {
                    if let Ok(info) = job_handle.query_extended_limit_info() {
                        ...
                        log::info!(
                            ...
                            info.peak_job_memory_used / (1024 * 1024)
                        );
                        ...
                        if info.peak_job_memory_used > MAX_JOB_MEMORY as u64 {
                            log::error!(
                                "CRITICAL_RESOURCE_BREACH: Memory limit exceeded during execution"
                            );
                        }
                        if info.peak_process_usage > MAX_CPU_RATE as u64 {
                            log::error!(
                                "CRITICAL_RESOURCE_BREACH: CPU limit exceeded during execution"
                            );
                        }
                    }
                }
                                ...
                                if name_str.starts_with("script_") && name_str.ends_with(".py") {
                                    ...
                                    mod tests {
                                        ...
                                        fn test_job_object_creation() {
                                            ...
                                            assert!(
                                                job.is_ok(),
                                                "Should create JobObjectManager successfully"
                                            );
                                        }
                                        ...
                                        async fn test_resource_limits() {
                                            let job = JobObjectManager::new()
                                                .expect("Failed to create JobObjectManager");
                                    ...
                                    "#;
                                            ...
                                            assert!(
                                                result.is_err(),
                                                "Should terminate CPU-intensive process"
                                            );
                                            assert!(
                                                result
                                                    .unwrap_err()
                                                    .contains("CRITICAL_RESOURCE_BREACH")
                                                    || result
                                                        .unwrap_err()
                                    ...
                                    "#;
                                            ...
                                            assert!(
                                                result.is_err(),
                                                "Should terminate memory-intensive process"
                                            );
                                            assert!(result
                                                .unwrap_err()
                                                .contains("CRITICAL_RESOURCE_BREACH"));
                                    ...
                                    "#;
                                            ...
                                            assert!(
                                                result.is_err(),
                                                "Should terminate long-running process"
                                            );
                                            assert!(result
                                                .unwrap_err()
                                                .contains("exceeded timeout"));
                                                ...
                                                std::fs::read_dir(std::env::temp_dir())
                                                    ...
                                                    .filter(|entry| {
                                                        if let Ok(entry) = entry {
                                                            ...
                                                            {
                                                                return name.starts_with("script_")
                                                                    && name.ends_with(".py");
                                                            }
                                                        }
                                                        false
