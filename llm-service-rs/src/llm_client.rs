...
impl PersonalityConfig {
    pub fn from_env() -> Self {
        Self {
            ...
            // Agent identity
            name: env::var("AGENT_NAME").unwrap_or_else(|_| "PHOENIX ORCH: The Ashen Guard Edition".to_string()),
            purpose: env::var("AGENT_PURPOSE").unwrap_or_else(|_| "To provide safe, helpful, and accurate assistance".to_string()),
        }
    }
...
impl LLMClient {
    ...
    pub async fn new() -> Self {
        ...
        log::info!("LLM client initialized with personality configuration: {:?}", personality);
        ...
        match llm_client.refresh_api_key().await {
            Ok(_) => log::info!("Successfully loaded API key for provider: {}", llm_client.provider),
            Err(err) => log::warn!("Failed to load API key from secrets: {}. API calls may fail.", err),
        }
                    ...
                    if let SecretsError::SecretNotFound(_) = err {
                        ...
                        match secrets.get_llm_api_key("default").await {
                            Ok(key) => {
                                let mut api_key = self.api_key.lock().await;
                                *api_key = key;
                                log::debug!("Using default API key (provider-specific key not found)");
                                return Ok(());
                            }
    ...
    pub async fn generate_text(&self, prompt: &str, system_prompt: Option<&str>) -> Result<String, LLMError> {
        ...
        log::info!("Preparing LLM request to {} (model: {})", self.api_url, self.model);
    ...
    async fn execute_request(&self, request_body: &ChatCompletionRequest) -> Result<String, LLMError> {
        ...
        let response = match self.client.post(&self.api_url)
        ...
        if !status.is_success() {
            ...
            return match status.as_u16() {
                ...
                429 => Err(LLMError::RateLimitExceeded(format!("Rate limit exceeded: {}", text))),
                // Server errors - retryable
                500 | 502 | 503 | 504 => Err(LLMError::ServerError(format!("Server error ({}): {}", status, text))),
                _ => Err(LLMError::UnknownError(format!("Unknown error ({}): {}", status, text))),
            };
        }
        ...
        match response_data {
            Ok(data) => {
                if let Some(choice) = data.choices.first() {
                    ...
                    Err(LLMError::ParseError("No choices returned in response".to_string()))
                }
            },
            Err(err) => Err(LLMError::ParseError(format!("Failed to parse response: {}", err))),
        }
    }
    
    // Convert to string error for compatibility with the main.rs implementation
    pub async fn generate_text_string(&self, prompt: &str, system_prompt: Option<&str>) -> Result<String, String> {
            ...
            Err(err) => {
                ...
                if let LLMError::InvalidRequest(msg) = &err {
                    if msg.contains("Unauthorized") || msg.contains("Invalid API key") {
                        log::warn!("Authentication error: {}. Attempting to refresh API key...", msg);
