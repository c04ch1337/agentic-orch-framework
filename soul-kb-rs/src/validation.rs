...
pub fn validate_priority(priority: i32) -> ValidationResult<i32> {
    if priority < MIN_PRIORITY || priority > MAX_PRIORITY {
        return Err(format!("Invalid priority: {}. Valid range is {} to {}", 
...
pub fn validate_value_metadata(metadata: &HashMap<String, String>) -> ValidationResult<HashMap<String, String>> {
...
pub fn validate_store_value_request(request: &StoreValueRequest) -> ValidationResult<StoreValueRequest> {
    // Check if value is present
    let value = match &request.value {
        Some(value) => value,
        None => return Err("Missing core value".to_string()),
    };
...
pub fn validate_ethics_check_request(request: &EthicsCheckRequest) -> ValidationResult<EthicsCheckRequest> {
    // Validate action
...
pub fn validate_min_priority(min_priority: i32) -> ValidationResult<i32> {
    ...
    if min_priority < 0 || min_priority > MAX_PRIORITY {
        return Err(format!("Invalid min priority: {}. Must be between 0 and {}", 
