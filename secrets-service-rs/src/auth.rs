    ...
    async fn initialize_defaults(&self) {
        ...
        let services_config = vec![
            ServiceCredentials {
                ...
                secret: env::var("LLM_SERVICE_SECRET")
                    .unwrap_or_else(|_| "default-llm-service-secret".to_string()),
                roles: vec!["llm-service".to_string(), "reader".to_string()],
            },
            ServiceCredentials {
                id: "api-gateway".to_string(),
                secret: env::var("API_GATEWAY_SECRET")
                    .unwrap_or_else(|_| "default-api-gateway-secret".to_string()),
