...
use crate::vault_client::{VaultClient, VaultOperations, VaultError, SecretMetadata};
...
use crate::secrets_service::{
    SecretRequest, SecretResponse, SetSecretRequest, OperationResponse,
    ListSecretsRequest, ListSecretsResponse, SecretMetadata as ProtoSecretMetadata,
    TokenRequest, TokenResponse, ServiceAuthRequest, ServiceAuthResponse
};
...
impl SecretsService for SecretsServiceImpl {
    ...
    async fn get_secret(
        ...
        if req.service_id.is_empty() || req.auth_token.is_empty() {
            return Err(Status::invalid_argument(
                "Missing service_id or auth_token"
            ));
        }
        
        // Check if the token is authorized to access this secret
        let is_authorized = self.auth_manager.is_authorized(
            &req.auth_token, 
            &format!("secret/{}", req.key), 
            "read"
        ).await;
        ...
        match self.vault_client.get_secret(&req.key, &req.auth_token).await {
            Ok(value) => {
            ...
            Err(err) => {
                match err {
                    ...
                    VaultError::AuthenticationError(msg) => {
    ...
    async fn set_secret(
        ...
        match self.vault_client.set_secret(
            &req.key, 
            &req.secret_value, 
            req.ttl, 
            req.metadata, 
    ...
    async fn delete_secret(
        ...
        match self.vault_client.delete_secret(&req.key, &req.auth_token).await {
            Ok(_) => {
                // Return success
                Ok(Response::new(OperationResponse {
                    success: true,
    ...
    async fn list_secrets(
        ...
        if !is_authorized {
            return Err(Status::permission_denied(
                "Not authorized to list secrets"
            ));
        }
        
        // List secrets from Vault
        match self.vault_client.list_secrets(&req.path_prefix, &req.auth_token).await {
            Ok(secrets) => {
    ...
    async fn generate_token(
        ...
        let token_data = self.auth_manager.authenticate_service(&req.service_id, &req.service_secret).await;
    ...
    async fn authenticate_service(
        ...
        log::info!(
            ...
            req.service_id, req.target_resource, req.action
        );
        ...
        let authorized = if authenticated {
            self.auth_manager.is_authorized(
                &req.auth_token, 
                &req.target_resource, 
                &req.action
            ).await
