        ...
        Ok(re) => {
            ...
            if start.elapsed() > Duration::from_millis(DEFAULT_REGEX_TIMEOUT_MS) {
                Err(ValidationError::SecurityThreat(format!(
                    "Regex matching timed out (potential ReDoS attack) for pattern: {}",
                    pattern
                )))
            } else {
                Ok(matches)
...
pub fn is_safe_regex(pattern: &str) -> ValidationResult<()> {
    ...
    let unsafe_constructs = [
        ...
        (
            r"\(.*\*.*\)+",
            "nested repetition with * inside capturing group with +",
        ),
        (
            r"\(.*\+.*\)+",
            "nested repetition with + inside capturing group with +",
...
pub fn validate_tokens(input: &str, tokens: &[&str], require_all: bool) -> ValidationResult<()> {
    ...
    if (require_all && matches.len() == tokens.len()) || (!require_all && !matches.is_empty()) {
        Ok(())
    } else if require_all {
        Err(ValidationError::PatternMismatch(format!(
            ...
            tokens
                .iter()
                .filter(|&&t| !matches.contains(&t))
                .collect::<Vec<_>>()
        )))
    } else {
        Err(ValidationError::PatternMismatch(format!(
            "Input doesn't contain any of the tokens: {:?}",
            tokens
        )))
    }
}
...
pub fn match_with_timeout<F>(matcher: F, timeout_ms: u64) -> ValidationResult<bool>
where
    F: FnOnce() -> bool,
{
    let start = Instant::now();
