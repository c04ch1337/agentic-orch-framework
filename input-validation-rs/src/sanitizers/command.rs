...
pub fn quote_argument(arg: &str) -> SanitizeResult<String> {
    ...
    if !has_special_chars && !arg.is_empty() {
        ...
        SanitizeResult::modified(result, Some("Quoted command argument".to_string()))
    }
}
...
pub fn remove_shell_metacharacters(command: &str) -> SanitizeResult<String> {
    ...
    if result == command {
        ...
        SanitizeResult::modified(result, Some("Removed shell metacharacters".to_string()))
    }
}
...
pub fn quote_command_args(command: &str) -> SanitizeResult<String> {
    ...
    if was_modified {
        SanitizeResult::modified(result, Some("Quoted command arguments".to_string()))
    } else {
        SanitizeResult::unmodified(command.to_string())
    }
}
...
pub fn command_to_args(command: &str) -> SanitizeResult<Vec<String>> {
    ...
    if parts.is_empty() {
        return SanitizeResult::modified(Vec::new(), Some("Empty command".to_string()));
    }
