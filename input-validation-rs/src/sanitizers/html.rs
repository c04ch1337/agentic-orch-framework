...
pub fn encode_html_entities(input: &str) -> SanitizeResult<String> {
    ...
    if was_modified {
        SanitizeResult::modified(
            result,
            Some("Encoded HTML entities".to_string())
        )
    } else {
...
pub fn decode_html_entities(input: &str) -> SanitizeResult<String> {
    ...
    if was_modified {
        SanitizeResult::modified(
            result,
            Some("Decoded HTML entities".to_string())
        )
    } else {
        SanitizeResult::unmodified(input.to_string())
    }
...
pub fn strip_html_tags(input: &str) -> SanitizeResult<String> {
    ...
    if result == input {
        ...
        SanitizeResult::modified(
            result,
            Some("Removed HTML tags".to_string())
        )
    }
}
...
pub fn sanitize_html(input: &str) -> SanitizeResult<String> {
    ...
    // Process HTML tags
    let sanitized = TAG_ATTR_REGEX.replace_all(&result, |caps: &regex::Captures| {
        let closing = &caps[1];
        ...
        format!("<{}{}{}{}>", closing, tag_name, sanitized_attrs, self_closing)
    }).to_string();
    ...
    if was_modified {
        SanitizeResult::modified(
            result,
            Some("Sanitized HTML content".to_string())
        )
    } else {
        SanitizeResult::unmodified(input.to_string())
    }
...
pub fn strict_html_sanitize(input: &str) -> SanitizeResult<String> {
    ...
    lazy_static! {
        static ref SCRIPT_REGEX: regex::Regex = regex::Regex::new(
            r"(?i)javascript:|data:|vbscript:|expression\(|@import"
        ).unwrap();
    }
    
    let result = SCRIPT_REGEX.replace_all(&decoded.sanitized, "[removed]").to_string();
    let was_modified = stripped.was_modified || decoded.was_modified || result != decoded.sanitized;
    
    if was_modified {
        SanitizeResult::modified(
            result,
            Some("Applied strict HTML sanitization".to_string())
        )
    } else {
        SanitizeResult::unmodified(input.to_string())
    }
...
pub fn escape_html_for_javascript(input: &str) -> SanitizeResult<String> {
    ...
    if was_modified {
        SanitizeResult::modified(
            result,
            Some("Escaped HTML for JavaScript".to_string())
        )
    } else {
        SanitizeResult::unmodified(input.to_string())
    }
...
mod tests {
    ...
    fn test_sanitize_html() {
        // Test with safe HTML
        let safe = "<p>Test <strong>bold</strong> text with <a href=\"https://example.com\">link</a></p>";
        ...
        let unsafe_attrs = "<p><a href=\"javascript:alert('XSS')\" onclick=\"evil()\">Bad Link</a></p>";
    ...
    fn test_escape_html_for_javascript() {
        ...
        assert!(result.was_modified);
        assert_eq!(result.sanitized, "Test \\x3Cscript\\x3Ealert(\\'XSS\\')\\x3C/script\\x3E");
    }
}