...
pub fn remove_path_traversal(path: &str) -> SanitizeResult<String> {
    ...
    let result = UNICODE_TRAVERSAL_REGEX.replace_all(&result, "/").to_string();
    ...
    if result == path {
        ...
        SanitizeResult::modified(
            result,
            Some("Removed path traversal sequences".to_string())
        )
    }
}
    ...
    match p.absolutize() {
        Ok(abs_path) => {
            ...
            if result == path {
                ...
                SanitizeResult::modified(
                    result,
                    Some("Normalized path".to_string())
                )
            }
        },
        Err(_) => {
...
pub fn confine_path(path: &str, base_dir: &str) -> SanitizeResult<String> {
    ...
    let abs_path = match p.absolutize() {
        Ok(ap) => ap,
        Err(_) => return SanitizeResult::modified(
            "".to_string(),
                ...
                for component in &components {
                    if component.as_os_str() == ".." {
                        return SanitizeResult::modified(
                            "".to_string(),
                            Some("Path attempts to traverse outside the base directory".to_string())
                        );
                    }
                }
...
pub fn sanitize_filename(filename: &str) -> SanitizeResult<String> {
    ...
    let original_len = result.len();
    result = result.trim_start_matches(|c| c == '.' || c == ' ')
