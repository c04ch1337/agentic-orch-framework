...
use input_validation_rs::{validate, validators::string::StringValidation, ValidationResult};
...
static STOPWORDS: Lazy<HashSet<&'static str>> = Lazy::new(|| {
    [
        ...
        "it", "its", "this", "that", "these", "those", "he", "she", "they", "we", "you", "i", "my",
        "your", "his", "her", "their", "our", "what", "which", "who", "whom", "whose", "when",
        "where", "why", "how", "not", "no", "so", "if", "then", "than", "too", "very", "just",
        "about", "into", "through", "during", "before", "after", "above", "below", "between",
        "under", "again", "further", "once", "here", "there", "all", "each", "few", "more", "most",
        "other", "some", "such", "only", "own", "same", "also", "both", "any", "nor",
    ]
    .iter()
    .cloned()
    .collect()
...
pub fn validate_text_input(text: &str) -> ValidationResult<()> {
    ...
    validate!(text, StringValidation::max_length(MAX_INPUT_LENGTH))?;
...
pub fn remove_stopwords(tokens: &[String]) -> Vec<String> {
    ...
    if tokens.len() > MAX_TOKENS {
        log::warn!(
            "Token list too large for stopword removal: {}",
            tokens.len()
        );
        return Vec::new();
    }
...
pub fn preprocess_query(query: &str) -> String {
    ...
    if query.len() > 4096 {
        // 4KB max for queries
        log::warn!(
            "Query too long for preprocessing: {} characters",
            query.len()
        );
        return String::new();
    }
