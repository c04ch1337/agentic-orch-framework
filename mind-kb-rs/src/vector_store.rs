...
impl FallbackStore {
    ...
    async fn search(&self, query_embedding: Vec<f32>, limit: usize) -> Vec<(String, f32, String)> {
        self.search_with_threshold(query_embedding, limit, 0.0)
            .await
    }
...
impl VectorStore {
    ...
    pub fn new_with_config(memory_limit: usize, retention_threshold: f32) -> Self {
        let collection_name =
            env::var("QDRANT_COLLECTION").unwrap_or_else(|_| COLLECTION_NAME.to_string());
    ...
    pub async fn store(
        ...
        validate_content(text).map_err(|e| format!("Content validation error: {}", e))?;

        validate_embedding(&embedding).map_err(|e| format!("Embedding validation error: {}", e))?;
    ...
    async fn prune_if_needed(&self) -> Result<(), String> {
        ...
        if count > self.memory_limit {
            ...
            log::info!(
                "Memory limit ({}) exceeded: current count is {}. Pruning {} memories.",
                self.memory_limit,
                count,
                to_prune
            );
            ...
            log::info!(
                "Memory pruning simulation: {} low-importance memories pruned",
                to_prune
            );
        }
    ...
    pub async fn process_decay(&self) -> Result<usize, String> {
        ...
        log::info!(
            "Temporal decay processing complete. Half-life: {} days",
    ...
    pub async fn search_with_config(
        ...
        match QDRANT_CLIENT.search_points(&search_request).await {
            Ok(response) => {
                ...
                let mut results: Vec<(String, f32, String)> = response
                    .result
                    .into_iter()
                    ...
                    .map(|point| {
                        ...
                        if decay_factor < 1.0 {
                            ...
                            tokio::spawn(async move {
                                ...
                                {
                                    Ok(_) => log::debug!(
                                        "Updated last_accessed timestamp for point {}",
                                        point_id
                                    ),
                                    Err(e) => log::warn!(
                                        "Failed to update last_accessed timestamp: {}",
                                        e
                                    ),
                                };
                            });
            ...
            Err(e) => {
                ...
                let fallback_results = self
                    .fallback_store
                    .search_with_threshold(query_embedding, limit as usize, threshold)
                    .await;
                Ok(fallback_results)
            }
        }
...
mod tests {
    ...
    fn test_decay_factor_calculation() {
        ...
        assert!(
            factor < 1.0,
            "Items at threshold (90 days) should have some decay"
        );
        ...
        assert!(
            factor <= 0.5 && factor > 0.45,
            "Items at 2x half-life should decay by ~50%"
        );
        ...
        assert!(
            factor < 0.25,
            "Very old items should have significant decay"
        );
    }
    ...
    fn test_timestamp_fields_in_storage() {
        ...
        rt.block_on(async {
            ...
            assert!(
                ...
                "Payload should contain timestamp field"
            );
            assert!(
                payload.contains_key("last_accessed"),
                "Payload should contain last_accessed field"
            );
    ...
    fn test_decay_integration() {
        ...
        rt.block_on(async {
            ...
            let result = store
                .search_with_config(query_embedding, limit, threshold)
                .await;

            // If we get an Ok result (even with empty results), our decay logic is at least not breaking
            assert!(
                result.is_ok(),
                "Search with decay functionality should not error"
            );
        });
    }
