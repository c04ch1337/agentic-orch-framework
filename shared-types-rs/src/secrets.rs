...
impl SecretManager {
    ...
    pub async fn get_secret(&self, path: &str) -> Result<String, SecretError> {
        ...
        loop {
            attempts += 1;
            info!(
                "Attempting to fetch secret from Vault (attempt {}/{})",
            ...
            match self.fetch_from_vault(path).await {
                Ok((value, version)) => {
                    debug!(
                        "Successfully fetched secret version {} from path: {}",
                        version, path
                    );
                    self.cache.set(path.to_string(), value.clone(), version);
                    info!(
                        "Secret retrieved and cached successfully for path: {}",
                        path
                    );
                    return Ok(value);
                }
                Err(e) => {
                    ...
                    warn!(
                        "Retry attempt {} for path {} failed: {}. Retrying in {:?}",
    ...
    pub async fn rotate_secret(&self, path: &str) -> Result<(), SecretError> {
        ...
        info!(
            "Writing new secret version {} for path: {}/{}",
            version + 1,
            mount,
            secret_path
        );
        ...
        self.cache
            .entries
            .remove(&format!("{}/{}", mount, secret_path));

        info!(
            "Secret rotation completed successfully for path: {}/{}",
            mount, secret_path
        );
        Ok(())
    }
    ...
    fn split_path(&self, full_path: &str) -> Result<(String, String), SecretError> {
        ...
        debug!(
            "Successfully split path into mount: {} and path: {}",
            parts[0], parts[1]
        );
    ...
    fn decrypt(&self, encrypted: &[u8]) -> Result<String, SecretError> {
        ...
        if encrypted.len() < 12 {
            ...
            return Err(SecretError::RotationError(
