...
impl ReflectionEngine {
    ...
    pub async fn reflect_on_action(
        ...
        debug!(
            "Reflecting on action: {}, outcome: {}, success: {}",
    ...
    async fn generate_lessons(
        ...
        if outcome.contains("timeout") || outcome.contains("timed out") {
            lessons.push(
                "Improve timeout handling for actions that may take longer to complete".to_string(),
            );
        }
        ...
        if outcome.contains("not found") || outcome.contains("missing") {
            lessons.push(
                "Validate resource existence before attempting operations that depend on them"
                    .to_string(),
            );
        }
    ...
    async fn suggest_improvements(
        ...
        if action.contains("connect") || action.contains("request") {
            improvements.push(
                "Implement exponential backoff and retry logic for network operations".to_string(),
            );
        }

        if action.contains("validate") || action.contains("verify") {
            improvements.push(
                "Add more comprehensive validation rules to prevent similar failures".to_string(),
            );
        }
    ...
    async fn suggest_optimizations(
        ...
        optimizations.push(
            "Consider if this operation could be cached to improve future performance".to_string(),
        );

        if action.contains("process") || action.contains("compute") || action.contains("calculate")
        {
            optimizations.push(
                "Evaluate if this operation could be parallelized for better efficiency"
                    .to_string(),
            );
        }
    ...
    pub async fn generate_negative_constraints(
        ...
        if !lessons.is_empty() && constraints.is_empty() {
            constraints.push(
                "Constraint: Exercise additional caution when similar conditions arise".to_string(),
            );
        }
    ...
    pub fn format_constraint_for_storage(&self, constraint: &str, context: &str) -> Constraint {
        let now = chrono::Utc::now();
