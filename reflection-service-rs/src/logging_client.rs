...
impl LoggingClient {
    ...
    pub async fn new() -> Self {
        ...
        match LoggingServiceClient::connect(addr.clone()).await {
            ...
            Err(e) => {
                warn!("Failed to connect to Logging Service at {}: {}. Using mock client.", addr, e);
                Self {
                    client: None,
                    service_name: "reflection-service".to_string(),
                    mock_mode: true,
    ...
    pub async fn log_improvement_event(
        ...
        let message = format!("Self-improvement event: Learned \"{}\" from action \"{}\"", 
                             lesson, action);
    ...
    pub async fn log_reflection_event(
        ...
        let message = format!("Reflection event: Action \"{}\" was {}", 
                             action, if success { "successful" } else { "unsuccessful" });
...
mod tests {
    ...
    async fn test_log_improvement_event() {
        ...
        let result = client.log_improvement_event(
            "test_action",
            "test_lesson",
            "test-123"
        ).await;
